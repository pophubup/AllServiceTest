...
namespace zWebCrawlingRepository
{
    public class CommonhealthCrawlingRepository
    {
        ...
        public byte[] GetBullshitContent()
        {
...
"慢性肝病及肝硬化","人體骨架",
    ...
    "附肢表面解剖"};
            ...
            topics.ToList().ForEach(topic => {
                ...
                var raw = new { Topic = topic, MinLen = 500 };
               var json = JsonConvert.SerializeObject(raw);
               var data = new StringContent(json, Encoding.UTF8, "application/json");
                for (int i = 0; i < 5; i++)
                {
                    var final = new HttpClient().PostAsync("https://api.howtobullshit.me/bullshit", data).GetAwaiter().GetResult();
                    List<string> everyStances = final.Content.ReadAsStringAsync().GetAwaiter().GetResult().Split('。').ToList().Select(g => Regex.Replace(g, @"\s+", "").Replace("&nbsp;", string.Empty).Replace("<br><br>", string.Empty).Replace("&rarr;", string.Empty).Trim()).ToList();
                    groups += String.Join("\r\n", everyStances) + "\r\n";
                    groups += Environment.NewLine;
                }
